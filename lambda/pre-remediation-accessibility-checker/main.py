...
def save_to_s3(bucket_name, file_key):
    ...
    print(
        f"Filename {file_key} | Uploaded {file_key} to {bucket_name} "
        f"at path {bucket_save_path} before remidiation"
    )
...
def lambda_handler(event, context):
    ...
    if not chunks:
        raise Exception("No chunks provided in event")

    first_chunk = chunks[0]
    s3_key = first_chunk.get("s3_key", None)

    if not s3_key:
        raise Exception("No s3_key found in first chunk")
